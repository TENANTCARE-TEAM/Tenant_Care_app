...
import { useGetRequestApprovedQuery } from "../../store/api/UsersSlice";

function TenantContent() {
  const {data: getApproved = [], error} = useGetRequestApprovedQuery()
       ...
       {getApproved.map((approved) => (
          ...
          <div className="flex flex-col gap-4 md:gap-7 w-full md:h-full">
            <div className="flex flex-col gap-3">
              ...
              <span className="text-sm text-[#acacac]">{approved.address}</span>
            </div>
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-2">
                <div className="flex gap-2">
                  ...
                  <h3 className="text-1xl text-gray-600">{approved.tenant_first_name} {approved.tenant_last_name}</h3>
                </div>
              </div>
            </div>
          </div>
